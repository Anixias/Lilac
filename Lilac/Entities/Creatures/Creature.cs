...
public abstract class Creature : Entity, IHittable, IBattleMember, IAttacker
{
    ...
    /// <summary>
    ///     Prepares this <see cref="Creature" /> for battle by rolling Initiative, setting up Stealth parameters, and
    ///     more.
    /// </summary>
	...
	{
		foreach (var effect in statusEffects) effect.OnTurnStarted();
	}

	public void TurnChanged()
	{
		foreach (var effect in statusEffects) effect.OnTurnChanged();
	}

	public void TurnEnded()
	{
		foreach (var effect in statusEffects) effect.OnTurnEnded();
	}
	...
	public RelationshipState GetRelationship(IRelationship? allegiance)
	{
		...
		if (allegiance is Allegiance allegianceLocal && allegiances.Contains(allegianceLocal))
	...
	protected void JoinAllegiance(Allegiance allegiance)
	{
		...
		defaultRelationship =
			(RelationshipState)Math.Min((int)defaultRelationship, (int)allegiance.DefaultRelationship);
	}
	...
	protected void LeaveAllegiance(Allegiance allegiance)
	{
		...
		foreach (var existingAllegiance in allegiances)
		{
			...
			newDefaultRelationship =
				(RelationshipState)Math.Min((int)newDefaultRelationship, (int)allegiance.DefaultRelationship);
		}
